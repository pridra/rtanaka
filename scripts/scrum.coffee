    ...
    message = (send) ->
        ...
        request.get
            url: "https://slack.com/api/users.list?token=#{process.env.HUBOT_SLACK_TOKEN}", (err, response, body) ->
                ...
                mem1 = for mem in JSON.parse(body)["members"] when mem["id"] == "UBHAXJD8V"
                mem2 = for mem in JSON.parse(body)["members"] when mem["id"] == "U1X8UV12N"
                mem3 = for mem in JSON.parse(body)["members"] when mem["id"] == "UBG88U4SW"
                mem4 = for mem in JSON.parse(body)["members"] when mem["id"] == "UBH3JT7V1"
                mem5 = for mem in JSON.parse(body)["members"] when mem["id"] == "U9XV9BZCK"
                mem6 = for mem in JSON.parse(body)["members"] when mem["id"] == "UBJ7T59V5"
                mem7 = for mem in JSON.parse(body)["members"] when mem["id"] == "UBLLAS3SQ"
                mem8 = for mem in JSON.parse(body)["members"] when mem["id"] == "UF847TJ7K"
                mem9 = for mem in JSON.parse(body)["members"] when mem["id"] == "UFA4E2E86"
                mem10 = for mem in JSON.parse(body)["members"] when mem["id"] == "UCLUECR5M"
                mem11 = for mem in JSON.parse(body)["members"] when mem["id"] == "U9TAHG70A"
