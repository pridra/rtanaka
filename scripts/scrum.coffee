    ...
    message = (send) ->
        ...
        request.get
            url: "https://slack.com/api/users.list?token=#{process.env.HUBOT_SLACK_TOKEN}", (err, response, body) ->
                ...
                member_list = for member_raw in JSON.parse(body)["members"] when !member_raw["is_bot"]
                mem1 = for mem in member_list when member_raw["id"] == "UBHAXJD8V"
                mem2 = for mem in member_list when member_raw["id"] == "U1X8UV12N"
                mem3 = for mem in member_list when member_raw["id"] == "UBG88U4SW"
                mem4 = for mem in member_list when member_raw["id"] == "UBH3JT7V1"
                mem5 = for mem in member_list when member_raw["id"] == "U9XV9BZCK"
                mem6 = for mem in member_list when member_raw["id"] == "UBJ7T59V5"
                mem7 = for mem in member_list when member_raw["id"] == "UBLLAS3SQ"
                mem8 = for mem in member_list when member_raw["id"] == "UF847TJ7K"
                mem9 = for mem in member_list when member_raw["id"] == "UFA4E2E86"
                mem10 = for mem in member_list when member_raw["id"] == "UCLUECR5M"
                mem11 = for mem in member_list when member_raw["id"] == "U9TAHG70A"
